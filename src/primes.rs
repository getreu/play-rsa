...
use num::bigint::{BigUint, RandBigInt, ToBigUint, BigInt, ToBigInt};
...
fn small_primes(bound: usize) -> Vec<usize> {
  let primes = primal::Primes::all().take(bound)
				.collect::<Vec<usize>>();
    primes
}
...
fn rabin_miller(candidate: &BigUint) -> bool {
  ...
  'witness_loop: for _ in 0..K {
    let basis = rand::thread_rng()
		.gen_biguint_range(&two, &(candidate-&one));
    let mut x = mod_exp(&basis, &d, candidate);

    if x == one ||  x == (candidate - &one) {break 'witness_loop}
    
    let mut s = s.clone();
    
    while  &s > &one {  // loop s-1 times
...
pub fn extended_gcd(a: &BigUint, b: &BigUint) -> 
    ...
    let (mut r, mut old_r) = 
         (b.to_bigint().unwrap(), a.to_bigint().unwrap());    
...
pub fn invmod(a: &BigUint, n: &BigUint) -> Option<BigUint> {
    ...
    let (mut t, mut new_t):(BigInt,BigInt) = 
         (Zero::zero(), One::one());
      
    let (mut r, mut new_r) = 
         (n.to_bigint().unwrap(), a.to_bigint().unwrap());    
...
#[cfg(test)]
mod test_primes {
  use super::{small_primes, mod_exp, is_prime, big_prime, rsa_prime, 
	  invmod, rewrite, extended_gcd};
  ...
  fn test_is_prime() {
    ...
    let known_composite: BigUint = 
		FromStr::from_str(known_composite_str).unwrap();
    ...
    let known_prime: BigUint = 
		FromStr::from_str(known_prime_str).unwrap();
