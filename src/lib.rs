...
impl fmt::Display for PublicKey {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
         try!(write!(f, "e=0x{}, ", self.e.to_bytes_be().to_hex()));
        try!(write!(f, "n=0x{}, ", self.n.to_bytes_be().to_hex()));
...
impl fmt::Display for PrivateKey {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        try!(write!(f, "d=0x{}, ", self.d.to_bytes_be().to_hex()));
...
pub fn gen_keys_default() -> (PublicKey, PrivateKey) {
  gen_keys(KeySizeT::DefaultKeySize, 
           PublicExponentT::DefaultExponent)
}

/// Generate RSA key-pair with given size and exponent.
pub fn gen_keys(key_size: KeySizeT, e: PublicExponentT) 
...
#[cfg(test)]
mod test_rsa {
  use super::{gen_keys_default, gen_keys};
  ...
  #[test]
  fn test_conversions() {
    assert_eq!("abcd".as_utf8_to_biguint(), 1633837924.to_biguint().unwrap()) ;
    assert_eq!("61626364".as_hex_to_biguint(), 1633837924.to_biguint().unwrap()); 
    assert_eq!(&1633837924.to_biguint().unwrap().to_string_as_utf8(), "abcd") ;
    assert_eq!(&1633837924.to_biguint().unwrap().to_string_as_hex(),"61626364") ;
  }
  ...
  fn test_encrypt_decrypt_five() {
    let (public, private) = gen_keys(KeySizeT::KeySize(512), 
				PublicExponentT::Exponent(5));
    let m = "my super secret";
    let encrypted = public.encrypt(m.clone());
    let decrypted = private.decrypt(&*encrypted);
    assert_eq!(m, &*decrypted);
